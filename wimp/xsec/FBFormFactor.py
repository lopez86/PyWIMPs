...
FBData = {
    ...
    (32,76):(4.081*u,np.array(0.37951e-1,0.57876e-1,0.15303e-2,-0.31822e-1,-0.76875e-2,
...
def FBFormFactor(FormFactor):
    ...
    def ff2(self,Q2):
        ...
        num = np.sum( [ (-1)^(i%2) * self._data[1][i-1] / 
                        (i*i*np.pi*np.pi - Q2*self._data[2]**2) 
                        for i in range(1,self._data[1].size+1] )

        denom = np.sum( [ (-1)^(i%2) * self._data[1][i-1] / (i*i*np.pi*np.pi) 
                          for i in range(1,self._data[1].size+1] )
